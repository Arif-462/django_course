...
@login_required
def sslcommerze_payment_getway(request, id, user_id, grand_total ):
    geteway_auth_details = PaymentGatewaySettings.objects.all().first()
    
    settings = {'store_id':geteway_auth_details.store_id,
                'store_pass':geteway_auth_details.store_pass, 
                'issandbox':True
                }

    print("heyyyyyyy ", settings)
